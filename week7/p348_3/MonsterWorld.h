...
public:
    ...
    ~MonsterWorld() { }  // 소멸자 (특별히 할 일 없음)

    // 몬스터를 게임에 추가하는 함수 (참조로 받기)
    ...
    void play(int maxwalk, int wait) {
        ...
        for (int i = 0; i < maxwalk; i++) {
            ...
            // 대기 시간 적용
            std::this_thread::sleep_for(std::chrono::milliseconds(wait));  // `Sleep` -> `std::this_thread::sleep_for`
        }
    }
};
