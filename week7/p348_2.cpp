...
public:
    ...
    // 기존 생성자: 초기화 리스트를 사용하지 않고 본문에서 멤버 변수에 값 할당
    ...
    // 새로운 생성자: 초 단위 실수 값(duration)을 받아서 시, 분, 초, 밀리초로 변환
    MyTime(double duration) {
        hours = static_cast<int>(duration / 3600);  // 시간을 계산 (3600초 = 1시간)
        duration = fmod(duration, 3600);  // 남은 초 계산
        minutes = static_cast<int>(duration / 60);  // 분 계산 (60초 = 1분)
        seconds = fmod(duration, 60);  // 남은 초 계산 (0~59초)
        milliseconds = static_cast<int>((seconds - static_cast<int>(seconds)) * 1000);  // 밀리초 계산
        seconds = static_cast<int>(seconds);  // 초는 정수로 변환
    }
...
int main() {
    ...
    MyTime converted(inputSeconds);  // double 타입의 초로 MyTime 객체 생성
    cout << "변환된 시간: ";
    converted.print();  // 변환된 시간을 출력

    MyTime time1 = converted;  // 사용자가 입력한 초로 time1 초기화
