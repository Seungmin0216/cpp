...
public:
    // 생성자: 몬스터의 이름, 아이콘, 초기 위치, 에너지 설정
    Monster(string n = "고블린", string i = "👹", int px = 0, int py = 0, int energy = 10)
        : name(n), icon(i), x(px), y(py), nItem(0), nEnergy(energy) {}
    ...
    Monster(const Monster& other) 
        : name(other.name), icon(other.icon), x(other.x), y(other.y), nItem(other.nItem), nEnergy(other.nEnergy) {}

    // 아이콘을 반환하는 함수
    string getIcon() const { return icon; }

    // 몬스터를 화면에 그리는 함수
    void draw(Canvas &canvas) { canvas.draw(x, y, icon.c_str()); }
    ...
    // 몬스터의 상태 (이름, 아이콘, 먹은 아이템 수, 에너지) 출력
    void print() { 
        cout << "\t" << name << icon << ": 아이템 개수 = " << nItem << " E: " << nEnergy << endl;
    }
